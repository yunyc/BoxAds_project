function updateState(oIdx, nextState, logTypeArr, alertMesg){
	...
	$.post("../print/updateState", { idx:oIdx, state:nextState, logTypeArr:logTypeArr, mesg:mesg }, function(result){
		if(result > 0){
			if(alertMesg != null){
				alert(alertMesg);
				location.reload();
...
function insertProcessLog(tag, alertMesg){		
	...
	$.post("../print/insertProcessLog", { idx:oIdx, logTypeArr:$(tag).attr("data-logType"), mesg:mesg, delivery:delivery }, function(result){
		if(result > 0){
			if(alertMesg != null){

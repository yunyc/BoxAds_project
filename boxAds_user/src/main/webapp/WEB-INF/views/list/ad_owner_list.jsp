	...
	<c:otherwise>
		<c:forEach items="${list}" var="vo" varStatus="status">	
			<tr onClick="location.href='#'">
				<td>${vo.insertDate}</td>
				<td><p class="ellipsis"><a href="../order/ad_order_detail?type=${type}&idx=${vo.idx}"><c:out value="${vo.adTitle}"/></a></p></td>
				<td><p class="ellipsis"><a href="../order/ad_order_detail?type=${type}&idx=${vo.idx}"><c:out value="${vo.boxTitle}"/></a></p></td>
				<td>${vo.name}</td>
				<td>${vo.sellDate}</td>
				...
				<c:choose>
					...
					<c:when test="${type eq 'B'}">
						<td class="align_r">${vo.totalPrice}</td>					
					</c:when>
				</c:choose>
				<td>
					<span class="step">
